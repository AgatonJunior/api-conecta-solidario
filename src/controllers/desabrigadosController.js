...
const getDesabrigados = async (req, res) => {
    try {
        ...
        return res.status(500).json({
            error: 'Erro ao listar desabrigados'
        });
    }
};
...
const postDesabrigados = async (req, res) => {
    ...
    try {
        ...
        if (abrigo.rows[0].vagas_livres < num_pessoas) {
            throw new Error('Vagas insuficientes');
        }

        const resultado = await client.query(
            `INSERT INTO desabrigados
        ...
        await client.query(
            `UPDATE abrigos 
            SET vagas_livres = vagas_livres - $1 
            WHERE id = $2`,
        ...
        return res.status(500).json({ error: 'Erro ao cadastra desabrigados' })
    } finally {
        client.release();
    }
};
...
const patchDesabrigaddos = async (req, res) => {
    ...
    try {
        ...
        const resultado = await client.query(
            `SELECT abrigo_id, num_pessoas, saida_em 
            FROM desabrigados 
            WHERE id = $1`,
            [id]
        );
        ...
        if (resultado.rows.length === 0) {
            throw new Error('Desabrigado não encontrado');
        }
        const desabrigado = resultado.rows[0];

        if (desabrigado.saida_em !== null) {
            throw new Error('Desabrigado já saiu do abrigo');
        }
        await client.query(
            `UPDATE desabrigados
            SET saida_em = NOW()
        ...
        return res.status(400).json({
            erro: 'Erro ao registrar a saída!'
        });
